...
<head>
    ...
    <style>
        ...
        .prod-badge { min-width: 150px; text-align: center; }
        .prod-action { min-width: 95px; text-align: right; }

        /* Buttons */
        .btn-cancel-item {
            background: #fff; color: #f44336; border: 1px solid #f44336;
            padding: 6px 14px; border-radius: 5px; font-size: 13px;
            font-weight: 600; cursor: pointer; white-space: nowrap;
        }
        .btn-cancel-item:hover { background: #f44336; color: #fff; }
        ...
        .btn-confirm-item:hover { background: #c2185b; }
...
<div class="wrapper">
            ...
            <span class="val">
                ...
                <c:choose>
                    <c:when test="${s == 'Thành công'}">
                        <span class="badge-status status-ok">
                            <i class="fa fa-check-circle"></i> ${s}
                        </span>
                    </c:when>
                    <c:when test="${s == 'Đã hủy'}">
                        <span class="badge-status status-huy">
                            <i class="fa fa-times-circle"></i> ${s}
                        </span>
                    </c:when>
                    <c:when test="${s == 'Đang giao hàng'}">
                        <span class="badge-status status-ship">
                            <i class="fa fa-truck"></i> ${s}
                        </span>
                    </c:when>
                    <c:when test="${s == 'Đã nhận hàng'}">
                        <span class="badge-status status-nhan">
                            <i class="fa fa-home"></i> ${s}
                        </span>
                    </c:when>
                    <c:otherwise>
                        <span class="badge-status status-cho">
    ...
    <div class="card-box">
        ...
        <c:set var="isShipping"  value="${order.status == 'Đang giao hàng' or order.status == 'Thành công' or order.status == 'Đã nhận hàng'}"/>
        <c:set var="isSuccess"   value="${order.status == 'Thành công' or order.status == 'Đã nhận hàng'}"/>
        <c:set var="isReceived"  value="${order.status == 'Đã nhận hàng'}"/>
        ...
        <ul class="timeline">
            ...
            <li>
                ...
                <span><strong>Đặt hàng</strong> – <em style="color:#999;">Chờ xác nhận từ cửa hàng</em></span>
            </li>

           
            <li>
                <c:choose>
                    <c:when test="${isShipping and not isCancelled}">
            ...
            <li>
                <c:choose>
                    <c:when test="${isReceived}">
                        <span class="tl-dot dot-nhan"></span>
                        <span><strong>Đã nhận hàng</strong></span>
                    </c:when>
                    <c:otherwise>
                        <span class="tl-dot dot-off"></span>
                        <span style="color:#bbb;">Đã nhận hàng</span>
                    </c:otherwise>
                </c:choose>
            </li>
    ...
    <div class="card-box">
        ...
        <c:forEach items="${details}" var="item">
            ...
            <c:set var="itemStatus"  value="${item.detail.status}"/>

            <c:choose>
                <c:when test="${itemStatus == 'Đang giao hàng'}"><c:set var="iBadge" value="status-ship"/></c:when>
                <c:when test="${itemStatus == 'Thành công'}">    <c:set var="iBadge" value="status-ok"/></c:when>
                <c:when test="${itemStatus == 'Đã nhận hàng'}">  <c:set var="iBadge" value="status-nhan"/></c:when>
                <c:when test="${itemStatus == 'Đã hủy'}">        <c:set var="iBadge" value="status-huy"/></c:when>
                <c:otherwise>                                    <c:set var="iBadge" value="status-cho"/></c:otherwise>
            </c:choose>
            ...
            <div class="product-row">
                ...
                <div class="prod-badge">
                    <span class="badge-status ${iBadge}">
                        <c:choose>
                            <c:when test="${itemStatus == 'Đang giao hàng'}"><i class="fa fa-truck"></i></c:when>
                            <c:when test="${itemStatus == 'Thành công'}"><i class="fa fa-check-circle"></i></c:when>
                            <c:when test="${itemStatus == 'Đã nhận hàng'}"><i class="fa fa-home"></i></c:when>
                            <c:when test="${itemStatus == 'Đã hủy'}"><i class="fa fa-times-circle"></i></c:when>
                ...
                <div class="prod-action">
                    <c:if test="${itemStatus == 'Chờ xác nhận'}">
                        <form action="${pageContext.request.contextPath}/user/orders/cancelItem"
                              method="post"
                              onsubmit="return confirm('Hủy sản phẩm này?')">
                            <input type="hidden" name="detailId" value="${item.detail.id}">
                            <input type="hidden" name="orderId"  value="${order.id}">
                            <button type="submit" class="btn-cancel-item">
                                <i class="fa fa-times"></i> Hủy
                            </button>
                        </form>
                    </c:if>
                    <c:if test="${itemStatus == 'Thành công'}">
                        <form action="${pageContext.request.contextPath}/user/orders/confirmItem"
                              method="post">
                            <input type="hidden" name="detailId" value="${item.detail.id}">
                            <input type="hidden" name="orderId"  value="${order.id}">
                            <button type="submit" class="btn-confirm-item">
                                <i class="fa fa-check"></i> Đã nhận
                            </button>
                        </form>
                    </c:if>
                    <c:if test="${itemStatus == 'Hoàn thành'}">
